...
class EncoderController:
        ...
        with self.lock:
            try:
                ...
                if self.connected:
                    ...
                    logger.info(
                        f"已成功連接到編碼器設備: 端口={port}, 波特率={baudrate}, 地址={address}")
    ...
    def start_monitoring(self, interval: float = 0.5) -> Tuple[bool, Union[Dict[str, Any], str]]:
        ...
        with self.lock:
                        ...
                        with self.lock:
                            ...
                            if position is None:
                                ...
                                if consecutive_errors > self.max_consecutive_errors:
                                    logger.error(
                                        f"連續讀取失敗 {consecutive_errors} 次，停止監測")
                                    break
                            ...
                            try:
                                # 先取得原始速度值
                                raw_speed_value = self.modbus_client.read_register(
                                    RegisterAddress.ENCODER_ANGULAR_SPEED)
                                # 轉換為帶符號數
            ...
            self._trigger_event("on_monitoring_started",
                                {"interval": interval})
    ...
    def stop_monitoring(self) -> Tuple[bool, Optional[str]]:
        ...
        with self.lock:
            ...
            if self.monitoring_thread.is_alive():
                logger.warning("監測線程未能在指定時間內停止")
                return False, "監測任務停止超時"
    ...
    def unregister_event_listener(self, event_name: str, callback: Callable) -> bool:
        """取消註冊事件監聽器
        ...
        with self.lock:
            if event_name in self.event_listeners and callback in self.event_listeners[event_name]:
                ...
                logger.debug(f"已取消註冊事件監聽器: {event_name}")
                return True
            return False
    ...
    def clear_event_listeners(self, event_name: Optional[str] = None) -> None:
        ...
        Args:
            event_name: 事件名稱，如果為None則清除所有事件監聽器
        """
        with self.lock:
            if event_name is None:
                self.event_listeners.clear()
                logger.debug("已清除所有事件監聽器")
            elif event_name in self.event_listeners:
                self.event_listeners[event_name].clear()
                logger.debug(f"已清除事件監聽器: {event_name}")
    ...
    def _start_connection_monitor(self) -> None:
        ...
        with self.lock:
            ...
            self.connection_monitor.add_connection_listener(
    ...
    async def read_position_and_speed_async(self) -> Tuple[Optional[int], Optional[float]]:
        ...
        loop = asyncio.get_event_loop()
        try:
            # 同步版本使用鎖來保證線程安全，但這可能在非同步操作中引入阻塞
            # 所以這裡使用一個單獨的鎖獲取操作來防止死鎖
            async def safe_read_position():
                if self.lock.acquire(timeout=2.0):
                    try:
                        result = await loop.run_in_executor(None, self.modbus_client.read_encoder_position)
                        if result is not None:
                            self._update_lap_count(result)
                        return result
                    finally:
                        self.lock.release()
                return None

            async def safe_read_speed():
                if self.lock.acquire(timeout=2.0):
                    try:
                        return await loop.run_in_executor(None, self.modbus_client.read_encoder_speed)
                    finally:
                        self.lock.release()
                return None

            # 並發執行兩個任務
            position, speed = await asyncio.gather(
                safe_read_position(),
                safe_read_speed(),
            ...
            if isinstance(position, Exception):
                logger.error(f"非同步讀取位置時出錯: {position}")
                position = None

            if isinstance(speed, Exception):
                logger.error(f"非同步讀取速度時出錯: {speed}")
                speed = None

            return position, speed
        except Exception as e:
            logger.error(f"非同步讀取位置和速度時出錯: {e}")
            return None, None
