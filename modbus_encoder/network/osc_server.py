...
class OSCServer:
    ...
    def __init__(
        ...
        self.dispatcher.map("/encoder/read_position",
                            self._encoder_read_position_handler)
        self.dispatcher.map("/encoder/read_speed",
                            self._encoder_read_speed_handler)
        self.dispatcher.map("/encoder/set_zero",
                            self._encoder_set_zero_handler)
        self.dispatcher.map("/encoder/start_monitor",
                            self._encoder_start_monitor_handler)
        self.dispatcher.map("/encoder/stop_monitor",
                            self._encoder_stop_monitor_handler)
        # 添加GPIO處理器
            ...
            if format_type.lower() == "json":
                ...
                if isinstance(data, list):
                    ...
                    if address == "/gpio/input":
                        params = [status, data.get(
                            "pin", 0), data.get("state", False)]
            ...
            if cmd == "connect":
                ...
                if client_address:
                    ...
                    if client_key in self.clients:
                        ...
                        # 返回確認
                        self.send_response(
                            {"status": "success", "message": f"已訂閱 {data_type} 數據"}, client_address)
                        return
                    else:
                        # 創建新客戶端記錄
                ...
                if client_address:
                    ...
                    if client_key in self.clients and "subscribe" in self.clients[client_key]:
                        if data_type in self.clients[client_key]["subscribe"]:
                            self.clients[client_key]["subscribe"].remove(
                                data_type)
                            logger.info(
                                f"客戶端 {client_address} 取消訂閱 {data_type} 數據")

                        # 返回確認
                        self.send_response(
                            {"status": "success", "message": f"已取消訂閱 {data_type} 數據"}, client_address)
                        return
    ...
    def broadcast(self, address: str, data: Any) -> int:
        ...
        logger.debug(f"Broadcasting to {len(self.clients)} clients: {address}")
        print(f"Broadcasting to {len(self.clients)} clients: {address}")

        with self.clients_lock:  # 使用鎖保護
            # If no clients, send to localhost:9000 as fallback
            if not self.clients:
                try:
                    client_addr = ("127.0.0.1", self.return_port)
                    client = udp_client.SimpleUDPClient(
                    ...
                    logger.debug(
                        f"Sent to fallback localhost:{self.return_port}")
                    print(f"Sent to fallback localhost:{self.return_port}")
                    success_count += 1
                    self.tx_count += 1
                except Exception as e:
                    logger.error(f"Failed sending to fallback: {e}")
                    print(f"Failed sending to fallback: {e}")
            ...
            for client_key, client_info in list(self.clients.items()):
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        json_data = json.dumps(data)
                        client.send_message(address, json_data)
                        logger.debug(f"Sent JSON to {client_addr}")
                        ...
                        logger.debug(f"Sent list to {client_addr}")
                    else:
                        # 所有其他類型轉為字符串
                        client.send_message(address, str(data))
                        logger.debug(f"Sent string to {client_addr}")
                    ...
                    print(f"Error broadcasting to {client_addr}: {e}")
                    self.error_count += 1
