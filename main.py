...
def handle_monitor_command(args):
    ...
    # 註冊資料更新事件監聽器
    controller.encoder_controller.register_event_listener("on_data_update", data_callback)
    
    # 開始監測
    result = controller.handle_command(f"start_monitor interval={interval} format={format_type}", "CLI")
...
def main():
    ...
    parser.add_argument("-i", "--interactive", action="store_true", help="啟動互動模式")
    ...
    monitor_parser.add_argument("-i", "--interval", type=float, default=0.5, help="監測間隔時間(秒)")
    monitor_parser.add_argument("-f", "--format", choices=["text", "json"], default="text", help="輸出格式")
